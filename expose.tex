...
\begin{frame}
  ...
  \uncover<2->{
  \item La puissance $N$ du fibré correspond au double du spin total $S$ et a un sens
  physique (taille des atomes).}
\uncover<3->{\item On veut étudier, dans la limite $N\to +\infty$, les
  vecteurs propres et valeurs propres des opérateurs de spins, et plus
  généralement des opérateurs de Toeplitz, et comprendre le lien avec
  des données géométriques sur la variété $M$ et l'énergie $f$ qu'on quantifie.}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Cas d'un symbole à plusieurs puits}
   ...
   \begin{minipage}[r]{0.65\linewidth}
     ...
     \uncover<3>{Ce qu'on minimise, c'est le $\mu$ de la hessienne en ce point.}
     \end{minipage}
\end{frame}
