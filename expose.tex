...
\uncover<3->{If $f\in L^{\infty}_{loc}$ has polynomial growth then $T_N(f)$ is an unbounded operator
with dense domain.}
...
\uncover<2->{\item Composition of smooth symbols: formal series
...
\begin{frame}
  ...
  \item With {\color{myorange} $g_N=(N/\pi)^ne^{-N|z|^2}$} one has
    ...
    \]\vspace{-2em}}
\uncover<4->{\item Equivalence between Toeplitz and $\Psi$DO
  calculus in the $C^{\infty}$ class.}
\uncover<5>{\item Toeplitz quantization is formulated {\color{myorange} directly in
  phase space}, and {\color{myorange} positive}.}

  \end{itemize}
\end{frame}

\section{Toeplitz quantization on manifolds}
...
\begin{array}{rcl}
 		T_N(f):H_N(M)&\to & H_N(M)\\
		u& \mapsto& \uncover<2->{S_N(}fu\uncover<2>{)}.
 		\end{array}
\end{center}
\end{frame}
...
\begin{frame}
  ...
  What are the energetic contributions of order $N^{-1}$ for the
  ground state?
  \begin{itemize}
